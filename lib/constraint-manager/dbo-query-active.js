 ...
 * This method queries active constraints based on time. 
 */

const utils = require("../cif-utils");
const { queryActiveConstraints } = require("./dbo-commons");

let responseBody = "Not Found";
let statusCode = 404; // Or 204 returns 'no content' and overrides response message

module.exports.main = async (state, time_start) => {
  // Ensures we have an integer representation of time_start
  const { error, data, message } = await queryActiveConstraints(state, parseInt(time_start));
